...
if (isset($_GET['search-product'])) {
    $search = urldecode($_GET['search-post']);
} else {
    $search = $_POST['search-input'];
}
$post = search_post($search, $total_num_row, $page);
                    ...
                    <ul class="pagination">
                        <?php if ($num_page != 0) { ?>
                            ...
                            <?php for ($i = 1; $i <= $num_page; $i++) {   ?>
                                <li class="page-item"><a class="page-link" href="?mod=post&controllers=index&action=search&search-post=<?php echo $search ?>&page=<?php echo $i ?>"><?php echo $i ?></a></li>
                            <?php } ?>
                        <?php } else {
                        } ?>
                    </ul>
